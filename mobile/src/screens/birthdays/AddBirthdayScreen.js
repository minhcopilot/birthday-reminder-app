...
export const AddBirthdayScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await BirthdayService.createBirthday(formData, userToken);
      
      if (result.success) {
        Alert.alert('Thành công', 'Đã thêm sinh nhật mới', [
      ...
      Alert.alert('Lỗi', 'Không thể thêm sinh nhật');
    } finally {
      setLoading(false);
    }
  };
